...
<div class="container cover">
    ...
    @php $no++; $id_blog_category = $blog_category->id_blg_cate @endphp
    <form action="{{ route('handle-Category', $id_blog_category) }}" method="post">@csrf
    <div class="row select_all add_new_hide">
        <div class="col">
            <div class="content">{{ $no }}</div>
        </div>
        <div class="col">
            <div class="content">{{ $blog_category->blg_category }}</div>
        </div>
        <div class="col text-center">
            <div class="content">
                @if($blog_category->homepage == 0) <input class="circle_ _red" type="submit" name="btnHomepage">
                @else <input class="circle_ _green" type="submit" name="btnHomepage"> @endif
            </div>
        </div>
        <div class="col text-center">
            <div class="content">
                @if($blog_category->display == 0) <input class="circle_ _red" type="submit" name="btnDisplay">
        ...
        {{-- checkboxes --}}
        <input class="form-check-input checkbox-tick" type="checkbox" name="ckbTick" value="{{ $id_blog_category }}">
    </div>
    </form>
    @endforeach
